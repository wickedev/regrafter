...
function Footer() {
  return <div>Footer</div>;
      ...
      if (result.ok && result.value.codes[0] !== undefined) {
        ...
        const headerIndex = firstCode.content.indexOf('<Header />');
        const conditionalIndex = firstCode.content.indexOf('{user && user.name && <UserProfile');
...
function Footer() {
  return <div>Footer</div>;
      ...
      if (result.ok && result.value.codes[0] !== undefined) {
        const firstCode = result.value.codes[0];
        expect(firstCode.content).toContain('items.map');
        expect(firstCode.content).toContain('<Footer />');

        // Verify the map expression moved after Footer
        const footerIndex = firstCode.content.indexOf('<Footer />');
        const mapIndex = firstCode.content.indexOf('items.map');
...
function Footer() {
  return <div>Footer</div>;
      ...
      if (result.ok && result.value.codes[0] !== undefined) {
        const firstCode = result.value.codes[0];
        expect(firstCode.content).toContain('data.filter');

        // Verify the chained expression moved before h1
        const h1Index = firstCode.content.indexOf('<h1>List</h1>');
        const filterIndex = firstCode.content.indexOf('data.filter');
...
function Footer() {
  return <div>Footer</div>;
      ...
      if (result.ok && result.value.codes[0] !== undefined) {
        const firstCode = result.value.codes[0];
        expect(firstCode.content).toContain('isLoading ?');
        expect(firstCode.content).toContain('<Footer />');

        // Verify the ternary expression moved after Footer
        const footerIndex = firstCode.content.indexOf('<Footer />');
        const ternaryIndex = firstCode.content.indexOf('isLoading ?');
...
function Footer() {
  return <div>Footer</div>;
      ...
      if (result.ok && result.value.codes[0] !== undefined) {
        const firstCode = result.value.codes[0];
        expect(firstCode.content).toContain('isOpen ?');

        // Verify the ternary moved before Header
        const headerIndex = firstCode.content.indexOf('<Header />');
        const ternaryIndex = firstCode.content.indexOf('isOpen ?');
...
function Footer() {
  return <div>Footer</div>;
      ...
      if (result.ok && result.value.codes[0] !== undefined) {
        ...
        expect(firstCode.content).toContain('isVisible &&');
        expect(firstCode.content).toContain('setIsVisible');
      }
    });
  });
});
