...
describe("isPositionSelector", () => {
  ...
  it("should return false if line is not a number", () => {
    ...
    expect(
      isPositionSelector(
        selector as { file: string; line: number; column: number }
      )
    ).toBe(false);
  });
...
describe("isPathSelector", () => {
  ...
  it("should return false if path is not a string", () => {
    ...
    expect(isPathSelector(selector as { file: string; path: string })).toBe(
      false
    );
  });
...
describe("isValidSelector", () => {
  it("should return true for valid position selector", () => {
    expect(isValidSelector({ file: "test.tsx", line: 10, column: 5 })).toBe(
      true
    );
  });

  it("should return true for valid path selector", () => {
    expect(isValidSelector({ file: "test.tsx", path: "Program.body[0]" })).toBe(
      true
    );
  });
